...
public class Controlleri
{
    ...
    public void scrapWrite(ServletContext  SC,String AuthCode[],Credential credits) throws Exception 
    {
        long currentTime = System.currentTimeMillis();
        
        if(spreadSheetApi.getSpreadSheetSize(SC,AuthCode,credits)>0)
        {
            scrapAndUpdate(convertLongToDate(currentTime),SC,AuthCode,credits);
        }
        else
        {
            ...
            {
                scrapAndUpdate(convertLongToDate(i),SC,AuthCode,credits);
            }
        }
    }
    
    private void scrapAndUpdate(String date,ServletContext  SC,String AuthCode[],Credential credits) throws Exception
    {
        List<Entity> entityList = scrapper.scrapTrainingData(date) ;
        spreadSheetApi.writeEntities(entityList,SC,AuthCode,credits);
    }
