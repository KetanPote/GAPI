...
public class SpreadSheetApi  
{
	...
	*/
    ...
    private Sheets getSheets(ServletContext  SC,String AuthCode[],Credential credits)throws Exception 
    {
        ...
        try 
        {
            ...
            Sheets service = new Sheets.Builder(HTTP_TRANSPORT, JSON_FACTORY, credits).setApplicationName("QuickStart").build();
    ...
    public List<ValueRange> writeEntities(List<Entity> entityList,ServletContext  SC,String AuthCode[],Credential credits)throws Exception 
    {     
    	int columnsSize = getSpreadSheetSize(SC,AuthCode,credits); 
        ...
        if (columnsSize == 0)  
        { 
            writeHeaders(SC,AuthCode,credits); 
            writeEntitiesToRaws(entityList, 2,SC,AuthCode,credits); 
        } 
        else 
        {
            List<String> lastdates = getColumnsByPosition(columnsSize+1,columnsSize+1,"C",SC,AuthCode,credits);
         
            String lastdate = lastdates.get(0);
            
            if(lastdate.equals(entityList.get(0).getValue(Constant.DATE)))
            {
                HashMap<String,Integer> usersPosition = getUsersPosition(entityList.get(0).getValue(Constant.DATE),columnsSize,entityList.size(),SC,AuthCode,credits);
                ...
                for(int i=0;i<entityList.size();i++)
                {
                    ...
                    if(position!=null)
                    {
                        List_VR.add(writeEntityToRaw(entity,position,SC,AuthCode,credits));
                    }
                    else
                    {
                        List_VR.add(writeEntityToRaw(entity,raw,SC,AuthCode,credits));
                        raw++;
                    }
                }
            }
            ...
            {
                List_VR=writeEntitiesToRaws(entityList, columnsSize+2,SC,AuthCode,credits);
            }
        }
        return List_VR;
    }
    private HashMap<String,Integer> getUsersPosition(String date, int size,int entitiesSize,ServletContext  SC,String AuthCode[],Credential credits)throws Exception
    {
        ...
        List<String>  dates = getColumnsByPosition(start,size,"C",SC,AuthCode,credits);
        List<String>  mails = getColumnsByPosition(start,size,"B",SC,AuthCode,credits);
    ...
    private List<ValueRange> writeEntitiesToRaws(List<Entity> entityList, int start,ServletContext  SC,String AuthCode[],Credential credits) 
    {
    	List<ValueRange> List_VR = new ArrayList<ValueRange>();         
        ...
        for (int i = 0; i < entityList.size(); i++) 
        {
            ...
            List_VR.add(writeData(entityWriteData, getRange("A", LAST_LETTER, rawCount,SC,AuthCode,credits),SC,AuthCode,credits));
        }
         
        return List_VR;
    }

    private ValueRange writeEntityToRaw(Entity entity, int position,ServletContext SC, String AuthCode[],Credential credits) 
    {
        List<List<Object>> entityWriteData = new ArrayList<>();
        List<Object> entityDataRow = new ArrayList<>();
       ...
       return writeData(entityWriteData, getRange("A", LAST_LETTER, position,SC,AuthCode,credits),SC,AuthCode,credits);

    }
    private ValueRange writeHeaders(ServletContext  SC,String AuthCode[],Credential credits) 
    {
        List<List<Object>> entityWriteData = new ArrayList<>();
        List<Object> entityDataRow = new ArrayList<>();
        ...
        return writeData(entityWriteData, getRange("A", LAST_LETTER, 1,SC,AuthCode,credits),SC,AuthCode,credits);
    }

  public String getDateByPosition(int position,ServletContext SC,String Data[],Credential credits) throws Exception
  {
//     .get(spreadsheetId, "A"+position-1+":A"+position)
    ...
    try 
    {
    	ValueRange response = getSheets(SC,Data,credits).spreadsheets()
              .values()
    ...
    public List<String> getColumnsByPosition(int start,int end, String column,ServletContext  SC,String AuthCode[],Credential credits)throws Exception 
    {
//     .get(spreadsheetId, "A"+position-1+":A"+position)
        ...
        try 
        {
            ValueRange response = getSheets(SC,AuthCode,credits).spreadsheets()
                    .values()
    ...
    public int getSpreadSheetSize(ServletContext  SC,String AuthCode[],Credential credits) throws Exception 
    {
        try 
        {
            
        	ValueRange response = getSheets(SC,AuthCode,credits).spreadsheets().values().get(spreadsheetId, "A2:A1000000").execute();
    ...
    private String getRange(String from, String to, int value,ServletContext  SC,String AuthCode[],Credential credits)
    {
        return from + value + ":" + to;
    }

    private ValueRange writeData(List<List<Object>> writeData, String range,ServletContext  SC,String AuthCode[],Credential credits) 
    {
        ...
        {        
            ...
            Sheets service = new Sheets.Builder(HTTP_TRANSPORT, JSON_FACTORY, credits) 
            ...
            System.out.println("Sucessfully Writing Data To The Google Sheet . . . !");
